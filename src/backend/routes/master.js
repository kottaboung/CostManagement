 ...
 *     description: Retrieve project details with optional filters by ProjectID and ProjectName. If no filters are provided, all projects will be returned.
 *     parameters:
 *       - name: ProjectID
 *         in: query
 ...
 *                   example: null
 */
router.get('/costdata/getdetail', (req, res) => {
    ...
    connection.query(baseQuery, [ProjectID, `%${ProjectName}%`].filter(Boolean), (err, projects) => {
      ...
      connection.query('SELECT ModuleID, ModuleName, ModuleAddDate, ModuleDueDate, ModuleActive, ProjectID FROM modules', (err, modules) => {
        ...
        // Query for employees and their connections to projects
